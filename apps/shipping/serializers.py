...
class ShippingGroupTariffSerializer(serializers.ModelSerializer):
    shipping_group_name = serializers.CharField(
        read_only=True, source="shipping_group.name"
    )
    shipping_type_code = serializers.CharField(
        read_only=True, source="shipping_type.code"
    )
